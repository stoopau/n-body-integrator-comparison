...
#   Velocity update (Leapfrog integration): 
#   Position update (Leapfrog integration): 
...
#   Kick: change in velocity due to acceleration 
#   Drift: change in position due to acceleration 

#Goal: 
#   Upgrade Euler integration to rk4 or leapfrog integration. 
#   Display potential and kinetic energies. 
...
velocity_y = np.array([0, 47900, 35000, 29780, 24100, 13070])
...
#Define acceleration computing func so leapfrog integration functions correctly. 
#Define energy function. 
#Leapfrog needs to be able to call twice per step in kick-drift-kick. 
#Loop to simulate. 
...
#For leapfrog integration -- restructure loop.
#Old: single kick + single drift
#New: kick-drift-kick. Higher accuracy. 

for step in range (num_steps): 
    
    #New acceleration computation. 
    acceleration_x, acceleration_y = compute_acc(x, y, mass)

    #Update velocity 
    velocity_x_half_kick = velocity_x + acceleration_x * (dt/2)
    velocity_y_half_kick = velocity_y + acceleration_y * (dt/2)
    
    #Update position 
    x = x + velocity_x_half_kick * dt
    y = y + velocity_y_half_kick * dt
    
    #Acceleration after half-kick, used for final kick. 
    acceleration_x, acceleration_y = compute_acc(x, y, mass)
    
    #Update velocity after second kick.
    velocity_x_full_kick = velocity_x_half_kick + acceleration_x * (dt/2)
    velocity_y_full_kick = velocity_y_half_kick + acceleration_y * (dt/2)

    #Update main v's to successfully get passed through func in loop. 
    velocity_x = velocity_x_full_kick
    velocity_y = velocity_y_full_kick
...
plt.savefig("results/leapfrog_orbits.png")
plt.show()
...
#Save energy results. 
plt.savefig("results/leapfrog_ke_pe.png")
plt.show()

#Verify energy drift has been minimized. 
total_energy = KE_history + PE_history  
plt.plot(total_energy / total_energy[0]-1)

#Save total energy results. 
plt.savefig("results/leapfrog_energy_drift.png")
plt.show()

#   As a result of switching from Euler integration to Leapfrog integration, I have reduced-
#   energy drift by a factor of ~1000x over the course of a 10-year, and 200-year, 6-body simulation. 
#   Both integrators show bounded (non-secular) oscillation at these timescales and step sizes. 